abbottApp.controller('salesAreaController', ['$rootScope','$filter','userDetailService','dashboardStateService','$scope', '$http','chartTypes','$state','chartConfigService','piSalesTargetService','$q','abbottConfigService', function($rootScope,$filter,userDetailService,dashboardStateService,$scope, $http, chartTypes, $state, chartConfigService, piSalesTargetService,$q,abbottConfigService) {
    ...
    $scope.reload = function(){
        ...
        $q.all(promisesArr).then(function(data){
            ...
            var dailySalesChart = chartConfigService.createChart(salesZoneData.salesDailyFlash.chartData, chartTypes.barChart, 'Daily Sales Flash', chartConfigService.barChartConfig({
                yAxis: 'Daily Sales'
            })); 
